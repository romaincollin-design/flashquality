...
const PROS_LIST = [{ id: 'trattoria', initials: 'TB', bg: '#FEF3C7', col: '#D97706', name: 'Trattoria Bella', sub: 'Restaurant \u00B7 Nice \u00B7 il y a 2j', rating: '4.7', hasFlash: true },{ id: 'carrel', initials: 'PC', bg: '#ECFDF5', col: '#059669', name: 'Plomberie Carrel', sub: 'Artisan \u00B7 Grasse \u00B7 il y a 2 sem.', rating: '4.9', hasFlash: false }]
const NOTIFS = [{ id: 'n1', initials: '\u26A1', ibg: '#FEF3C7', icol: '#D97706', title: 'Trattoria Bella \u00B7 Offre flash', desc: 'Pizza \u201330% ce soir jusqu\u2019\u00E0 21h.', time: 'Il y a 12 min', dot: '#FF4D6D' },{ id: 'n2', initials: '\u{1F3C6}', ibg: '#EDE8FF', icol: '#7C5CFC', title: 'FlashQuality', desc: 'Niveau 2 atteint ! Tests produits d\u00E9bloqu\u00E9s.', time: 'Il y a 1h', dot: A }]
const SUBS = [{ icon: '\u{1F3E0}', ibg: '#FFF1EB', name: 'Assurance habitation \u2014 AXA', price: '72\u20AC/mois', date: '27/04/2026', countdown: 'J-14', countdownColor: '#DC2626' },{ icon: '\u{1F4E1}', ibg: '#EEF4FF', name: 'Box Internet \u2014 Orange', price: '36\u20AC/mois', date: '15/09/2026', countdown: 'J-155', countdownColor: '#10B981' },{ icon: '\u26A1', ibg: '#F0FDF9', name: '\u00C9lectricit\u00E9 \u2014 EDF', price: '89\u20AC/mois', date: '01/01/2027', countdown: 'J-263', countdownColor: '#10B981' }]
const DEP_HISTORY = [{ icon: '\u{1F37D}\uFE0F', ibg: '#F3F0FF', name: 'Trattoria Bella', cat: 'Restaurant \u00B7 Hier', amount: '48,50 \u20AC' },{ icon: '\u2702\uFE0F', ibg: '#FFF0FB', name: 'Studio Coiff', cat: 'Coiffeur \u00B7 Il y a 3j', amount: '65,00 \u20AC' },{ icon: '\u26FD', ibg: '#E0FDF4', name: 'Total \u00C9nergies', cat: 'Essence \u00B7 Il y a 5j', amount: '82,30 \u20AC' }]

function fmtTimer(s: number) { const h=Math.floor(s/3600),m=Math.floor((s%3600)/60),sc=s%60; return (h?h+'h ':'')+String(m).padStart(2,'0')+'m '+String(sc).padStart(2,'0')+'s' }
function fmtShort(s: number) { const h=Math.floor(s/3600),m=Math.floor((s%3600)/60); return (h?h+'h ':'')+String(m).padStart(2,'0')+'m' }

function SHD({ title, onBack, right }: { title: string; onBack?: () => void; right?: React.ReactNode }) {
  return (
    <div style={{ padding: '12px 14px 8px', display: 'flex', alignItems: 'center', gap: 10, background: 'white', borderBottom: '2px solid #F0EDE8', flexShrink: 0 }}>
...
export default function ClientApp() {
  ...
  const [visibility, setVisibility] = useState<'pro' | 'public'>('pro')
  const [showCel, setShowCel] = useState(false)
  const [showDepCel, setShowDepCel] = useState(false)
  const [timerSecs, setTimerSecs] = useState(6138)
  const [aiLoading, setAiLoading] = useState(false)
  const [aiDone, setAiDone] = useState(false)
  const [depPresta, setDepPresta] = useState('')
  const [depAmount, setDepAmount] = useState('')
  const [depCat, setDepCat] = useState('\u{1F37D}\uFE0F Resto')
  ...
  const navTo = (nav: NavTab, s: Screen) => { setActiveNav(nav); setScreen(s) }

  const simulateAI = () => {
    setAiLoading(true); setAiDone(false)
    setTimeout(() => { setDepPresta('Trattoria Bella'); setDepAmount('48.50'); setAiLoading(false); setAiDone(true) }, 1800)
  }

  function AC({ hot, onClick, badge, badgeColor, icon, title, sub, accentColor }: { hot?: boolean; onClick: () => void; badge?: string; badgeColor?: string; icon: string; title: string; sub: string; accentColor?: string }) {
    const col = accentColor || (hot ? A : undefined)
    return (
      <div onClick={onClick} style={{ background: hot ? AL : accentColor ? '#D1FAF0' : 'white', borderRadius: RC, padding: '14px 12px 12px', cursor: 'pointer', border: `2px solid ${hot ? A : accentColor || '#E8E4DC'}`, position: 'relative', overflow: 'hidden', boxShadow: '0 2px 10px rgba(0,0,0,.07)', boxSizing: 'border-box' }}>
        {badge && <div style={{ position: 'absolute', top: 10, right: 10, background: badgeColor || A, color: 'white', fontSize: 9, fontWeight: 900, padding: '2px 7px', borderRadius: 100 }}>{badge}</div>}
        <div style={{ fontSize: 28, lineHeight: 1, marginBottom: 5 }}>{icon}</div>
        <div style={{ fontSize: 13, fontWeight: 900, color: col || '#1A1033' }}>{title}</div>
    ...
    <div style={{ flex: 1, overflowY: 'auto' } as React.CSSProperties}>
      <div style={{ background: A, padding: '18px 16px', position: 'relative', overflow: 'hidden', borderRadius: RC, margin: '12px 12px 0', border: '2.5px solid rgba(255,255,255,.15)', boxShadow: `0 8px 28px rgba(124,92,252,.45), 0 2px 0 ${AD}` }}>
        ...
        <div style={{ display: 'flex', gap: 7 }}>
          <div style={{ display: 'inline-flex', alignItems: 'center', gap: 4, background: 'rgba(255,255,255,.22)', border: '1px solid rgba(255,255,255,.35)', borderRadius: 100, padding: '5px 11px', fontSize: 10, color: 'white', fontWeight: 800 }}>\u2713 V\u00E9rifi\u00E9</div>
          <div style={{ display: 'inline-flex', alignItems: 'center', gap: 4, background: 'rgba(255,255,255,.22)', border: '1px solid rgba(255,255,255,.35)', borderRadius: 100, padding: '5px 11px', fontSize: 10, color: 'white', fontWeight: 800 }}>{'\u{1F37D}\uFE0F'} {PRO.badge}</div>
        </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, padding: '12px 12px 0' }}>
        ...
        <AC onClick={() => go('offers')} badge="\u26A1 2" badgeColor="#FF4D6D" icon="\u26A1" title="Offres flash" sub="2 actives" />
        <AC onClick={() => go('contact')} icon="\u{1F4AC}" title="Contacter" sub="Message \u00B7 Appel" />
        <AC onClick={() => navTo('depenses', 'depenses')} icon="\u{1F9FE}" title="D\u00E9penses" sub="Photo ticket \u00B7 TVA" accentColor="#10B981" />
      </div>
      <div onClick={() => go('offers')} style={{ margin: '10px 12px 0', background: `linear-gradient(135deg, ${A}, ${AD})`, borderRadius: RC, padding: '14px 16px', display: 'flex', alignItems: 'center', gap: 12, cursor: 'pointer', boxShadow: '0 8px 24px rgba(124,92,252,.4)', border: '1.5px solid rgba(255,255,255,.1)' }}>
        ...
        <div style={{ flex: 1 }}>
          ...
          <div style={{ fontSize: 10, color: 'rgba(255,255,255,.55)', marginTop: 3, fontWeight: 700 }}>{'\u23F1'} Expire dans {fmtTimer(timerSecs)}</div>
        </div>
      ...
      <div style={{ padding: '14px 12px 0' }}>
        ...
        {PROS_LIST.map(p => (
          <div key={p.id} onClick={() => go('contact')} style={{ ...CARD, display: 'flex', alignItems: 'center', gap: 11, padding: '12px 13px', marginBottom: 9, cursor: 'pointer', boxShadow: '0 2px 10px rgba(0,0,0,.07)', position: 'relative' }}>
    ...
    <div style={{ display: 'flex', flexDirection: 'column', flex: 1, overflow: 'hidden' }}>
      ...
      <div style={{ flex: 1, overflowY: 'auto' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 9, padding: '0 13px 12px' }}>
          {RATINGS.map((r, i) => (
            <div key={i} onClick={() => setSelectedRating(i)} style={{ background: selectedRating === i ? AL : 'white', border: `2.5px solid ${selectedRating === i ? A : '#F0EDE8'}`, borderRadius: RC, padding: '13px 8px', textAlign: 'center', cursor: 'pointer', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4, boxSizing: 'border-box' }}>
              <div style={{ fontSize: 28, lineHeight: 1 }}>{r.emoji}</div>
              <div style={{ fontSize: 10, fontWeight: 800, color: selectedRating === i ? A : '#B0ADA8' }}>{r.label}</div>
            </div>
          ))}
        </div>
        ...
        <div style={{ display: 'flex', gap: 8, padding: '0 13px 10px' }}>
          {(['pro', 'public'] as const).map(v => (
            <div key={v} onClick={() => setVisibility(v)} style={{ flex: 1, padding: '8px 12px', borderRadius: RC, border: `2px solid ${visibility === v ? A : '#F0EDE8'}`, background: visibility === v ? AL : 'white', textAlign: 'center', fontSize: 12, fontWeight: 800, color: visibility === v ? A : '#B0ADA8', cursor: 'pointer', boxSizing: 'border-box' }}>
              {v === 'pro' ? '\u{1F512} Pro seulement' : '\u{1F30D} Communaut\u00E9'}
            </div>
          ))}
        </div>
        <div style={{ padding: '0 13px 16px' }}>
          <button onClick={() => { if (selectedRating !== null) setShowCel(true) }} style={{ width: '100%', padding: 14, borderRadius: 100, background: selectedRating !== null ? A : '#D0CEC9', color: 'white', fontSize: 14, fontWeight: 900, border: 'none', cursor: selectedRating !== null ? 'pointer' : 'not-allowed', fontFamily: 'inherit', marginBottom: 10, boxSizing: 'border-box' }}>Envoyer {'\u{1F680}'}</button>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '12px 13px' }}>
        <div style={{ ...CARD, border: `2.5px solid ${A}`, marginBottom: 10, overflow: 'hidden' }}>
          ...
          <div style={{ padding: '0 13px 12px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <div style={{ background: '#FFE8EC', color: '#FF4D6D', fontSize: 11, fontWeight: 800, padding: '4px 10px', borderRadius: 100 }}>\u23F1 {fmtShort(timerSecs)}</div>
        ...
        <div style={{ background: AL, borderRadius: RC, padding: '14px 13px', border: '2.5px dashed rgba(124,92,252,.28)', textAlign: 'center' }}>
          ...
          <div style={{ fontSize: 11, color: '#A8A5A0', fontWeight: 700, marginBottom: 12, lineHeight: 1.5 }}>Notifi\u00E9\u00B7e quand {PRO.name} publie une offre</div>
    ...
    <div style={{ display: 'flex', flexDirection: 'column', flex: 1, overflow: 'hidden' }}>
      ...
      <div style={{ flex: 1, overflowY: 'auto' }}>
        <div style={{ margin: '12px 13px 0', ...CARD, padding: 14 }}>
          ...
          {([['\u{1F4DE}', PRO.tel],['\u{1F4CD}', PRO.addr],['\u{1F553}', PRO.hours]] as [string,string][]).map(([ico, val]) => (
        ...
        {showMsg && (
          <div style={{ margin: '10px 13px 0' }}>
            <textarea value={msgText} onChange={e => setMsgText(e.target.value)} rows={3} placeholder="Votre message..." style={{ width: '100%', border: '2.5px solid #F0EDE8', borderRadius: RC, padding: '11px 13px', fontSize: 13, fontWeight: 700, fontFamily: 'inherit', resize: 'none', outline: 'none', boxSizing: 'border-box' }} />
        ...
        <div style={{ padding: '12px 13px 14px' }}>
          <div style={{ fontSize: 10, fontWeight: 900, color: '#B0ADA8', textTransform: 'uppercase', letterSpacing: '.08em', marginBottom: 9 }}>\u26A1 Offre active en ce moment</div>
    ...
    <div style={{ display: 'flex', flexDirection: 'column', flex: 1, overflow: 'hidden' }}>
      <div style={{ background: 'linear-gradient(135deg,#10B981,#059669)', padding: '18px 18px 16px', flexShrink: 0 }}>
        ...
        <div style={{ display: 'flex', gap: 12 }}>
          {([['324\u20AC','Ce mois'],['12','Tickets'],['6','Cat\u00E9gories']] as [string,string][]).map(([v,l]) => (
            <div key={l} style={{ background: 'rgba(255,255,255,.15)', borderRadius: 12, padding: '8px 12px', textAlign: 'center' }}>
      ...
      <div style={{ flex: 1, overflowY: 'auto', paddingBottom: 16 }}>
        <div style={{ margin: '12px 12px 0', background: 'white', borderRadius: RC, border: '2px solid #E8E4DC', padding: 14 }}>
          ...
          <div style={{ display: 'flex', gap: 6, alignItems: 'flex-end', height: 72 }}>
            {([['55px',A,'Resto'],['38px','#EC4899','Beaut\u00E9'],['28px','#F59E0B','Sortie'],['22px','#06B6D4','Coiffeur'],['42px','#10B981','Essence'],['18px','#B0ADA8','Autre']] as [string,string,string][]).map(([h,c,l]) => (
        ...
        <div style={{ margin: '10px 12px 0' }}>
          <div onClick={() => fileRef.current?.click()} style={{ background: 'white', borderRadius: RC, border: '2.5px dashed #D0CEC9', padding: '20px 16px', textAlign: 'center', cursor: 'pointer' }}>
          ...
          <div style={{ textAlign: 'center', marginTop: 8 }}>
            <button onClick={simulateAI} style={{ display: 'inline-flex', alignItems: 'center', gap: 6, background: aiDone ? 'linear-gradient(135deg,#10B981,#34D399)' : `linear-gradient(135deg,${A},${AD})`, color: 'white', fontSize: 12, fontWeight: 900, padding: '8px 16px', borderRadius: 100, border: 'none', cursor: 'pointer', fontFamily: 'inherit', opacity: aiLoading ? 0.7 : 1 }}>
              {aiLoading ? '\u2726 Analyse en cours\u2026' : aiDone ? '\u2726 Extraction compl\u00E8te !' : '\u2726 Identifier avec l\u2019IA'}
            </button>
          </div>
        </div>
        {aiDone && (
          <div style={{ margin: '10px 12px 0', background: 'white', borderRadius: RC, border: `2.5px solid ${A}`, overflow: 'hidden' }}>
            ...
            <div style={{ padding: '12px 13px' }}>
              {([['Prestataire','Trattoria Bella'],['Cat\u00E9gorie','\u{1F37D}\uFE0F Restauration'],['Date', new Date().toLocaleDateString('fr-FR')],['Montant','48,50 \u20AC']] as [string,string][]).map(([l,v]) => (
        ...
        <div style={{ margin: '10px 12px 0', background: 'white', borderRadius: RC, border: '2px solid #E8E4DC', padding: 14 }}>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 10 }}>
            <div>
              ...
              <input value={depPresta} onChange={e => setDepPresta(e.target.value)} placeholder="Nom\u2026" style={{ width: '100%', border: '2px solid #F0EDE8', borderRadius: 12, padding: '8px 10px', fontSize: 13, fontWeight: 700, color: '#1A1033', fontFamily: 'inherit', outline: 'none', boxSizing: 'border-box' }} />
            </div>
            <div>
              <div style={{ fontSize: 10, fontWeight: 900, color: '#B0ADA8', textTransform: 'uppercase', letterSpacing: '.06em', marginBottom: 5 }}>Montant</div>
              <input value={depAmount} onChange={e => setDepAmount(e.target.value)} type="number" step="0.01" placeholder="0,00 \u20AC" style={{ width: '100%', border: '2px solid #F0EDE8', borderRadius: 12, padding: '8px 10px', fontSize: 13, fontWeight: 700, fontFamily: 'inherit', outline: 'none', boxSizing: 'border-box' }} />
            </div>
          </div>
          <div style={{ marginBottom: 12 }}>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
              {['\u{1F37D}\uFE0F Resto','\u{1F485} Beaut\u00E9','\u{1F3AD} Sortie','\u2702\uFE0F Coiffeur','\u26FD Essence','\u{1F3E5} Sant\u00E9','\u{1F4E6} Autre'].map(cat => (
                <div key={cat} onClick={() => setDepCat(cat)} style={{ padding: '6px 12px', borderRadius: 100, border: `2px solid ${depCat === cat ? A : '#E8E4DC'}`, background: depCat === cat ? AL : 'white', fontSize: 12, fontWeight: 800, color: depCat === cat ? A : '#B0ADA8', cursor: 'pointer' }}>{cat}</div>
              ))}
            </div>
          </div>
          <button onClick={() => setShowDepCel(true)} style={{ width: '100%', padding: 12, borderRadius: 100, background: '#10B981', color: 'white', fontSize: 13, fontWeight: 900, border: 'none', cursor: 'pointer', fontFamily: 'inherit', boxSizing: 'border-box' }}>{'\u{1F4BE}'} Enregistrer</button>
        </div>
        <div style={{ margin: '12px 12px 0' }}>
          ...
          {DEP_HISTORY.map(d => (
            <div key={d.name} style={{ ...CARD, display: 'flex', alignItems: 'center', gap: 10, padding: '12px 13px', marginBottom: 8 }}>
              ...
              <div style={{ fontSize: 14, fontWeight: 900, color: '#1A1033' }}>{d.amount}</div>
            </div>
          ))}
        </div>
      </div>
  ...
  const CarnetScreen = () => (
    <div style={{ display: 'flex', flexDirection: 'column', flex: 1, overflow: 'hidden' }}>
      ...
      <div style={{ display: 'flex', gap: 6, padding: '10px 13px 0', overflowX: 'auto', flexShrink: 0 }}>
        {[{id:'all',label:'Tous'},{id:'resto',label:'\u{1F37D}\uFE0F Resto'},{id:'beaute',label:'\u{1F485} Beaut\u00E9'},{id:'artisan',label:'\u{1F527} Artisan'}].map(c => (
          <div key={c.id} onClick={() => setCarnetFilter(c.id)} style={{ whiteSpace: 'nowrap', padding: '6px 13px', borderRadius: 100, border: `2px solid ${carnetFilter === c.id ? 'transparent' : '#F0EDE8'}`, background: carnetFilter === c.id ? A : 'white', fontSize: 11, fontWeight: 800, color: carnetFilter === c.id ? 'white' : '#B0ADA8', cursor: 'pointer' }}>{c.label}</div>
        ))}
      </div>
      ...
      <div style={{ flex: 1, overflowY: 'auto' }}>
        <div style={{ background: A, padding: '24px 16px 20px', position: 'relative', overflow: 'hidden', textAlign: 'center' }}>
          ...
          <div style={{ display: 'flex', gap: 12, justifyContent: 'center', marginTop: 16, flexWrap: 'wrap' }}>
            {([['6','avis'],['3','pros suivis'],['240','XP total'],['12','tickets']] as [string,string][]).map(([v,l]) => (
              <div key={l} style={{ background: 'rgba(255,255,255,.15)', borderRadius: 12, padding: '8px 12px', textAlign: 'center' }}>
        ...
        <div style={{ padding: '8px 13px 14px', display: 'flex', flexDirection: 'column', gap: 7 }}>
          {([
            {icon:'\u{1F331}',level:'Niveau 1 \u00B7 Nouveau',sub:'1er scan \u00B7 Offres flash standards',xp:'0 XP',current:false,opacity:1},
            {icon:'\u{1F3C6}',level:'Niveau 2 \u00B7 R\u00E9gulier \u2190 Tu es ici',sub:'3 avis + 5 scans \u00B7 Tests produits',xp:'100 XP',current:true,opacity:1},
            {icon:'\u{1F48E}',level:'Niveau 3 \u00B7 Fid\u00E8le',sub:'10 avis + 3 pros \u00B7 Exp\u00E9riences exclusives',xp:'500 XP',current:false,opacity:0.7},
            {icon:'\u{1F396}\uFE0F',level:'Niveau 4 \u00B7 Ambassadeur',sub:'Invit\u00E9 par un pro \u00B7 Panel testeur certifi\u00E9',xp:'Sur invitation',current:false,opacity:0.5},
          ] as {icon:string;level:string;sub:string;xp:string;current:boolean;opacity:number}[]).map((lvl,i) => (
            <div key={i} style={{ background: lvl.current ? AL : 'white', borderRadius: RC, border: `2.5px solid ${lvl.current ? A : '#F0EDE8'}`, padding: '11px 13px', display: 'flex', alignItems: 'center', gap: 10, opacity: lvl.opacity }}>
              <div style={{ width: 38, height: 38, borderRadius: 10, background: lvl.current ? A : '#F0EDE8', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 18, flexShrink: 0 }}>{lvl.icon}</div>
              <div style={{ flex: 1 }}>
                <div style={{ fontSize: 13, fontWeight: 900, color: lvl.current ? A : '#1A1033' }}>{lvl.level}</div>
                <div style={{ fontSize: 11, color: lvl.current ? A : '#B0ADA8', opacity: lvl.current ? 0.7 : 1, fontWeight: 700, marginTop: 2 }}>{lvl.sub}</div>
              </div>
              <div style={{ fontSize: 11, fontWeight: 700, color: lvl.current ? A : '#B0ADA8' }}>{lvl.xp}</div>
            </div>
          ))}
        </div>
      </div>
  ...
  const NotifsScreen = () => (
    <div style={{ display: 'flex', flexDirection: 'column', flex: 1, overflow: 'hidden' }}>
      ...
      <div style={{ flex: 1, overflowY: 'auto' }}>
        ...
        {NOTIFS.map(n => (
          <div key={n.id} style={{ ...CARD, margin: '0 13px 8px', display: 'flex', alignItems: 'flex-start', gap: 10, padding: '12px 13px' }}>
            <div style={{ width: 40, height: 40, borderRadius: RC, background: n.ibg, color: n.icol, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 16, fontWeight: 900, flexShrink: 0 }}>{n.initials}</div>
            <div style={{ flex: 1 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 3 }}>
                <div style={{ width: 7, height: 7, borderRadius: '50%', background: n.dot, flexShrink: 0 }} />
                <div style={{ fontSize: 12, fontWeight: 900, color: '#1A1033' }}>{n.title}</div>
              </div>
  ...
  const AbonnementsScreen = () => (
    <div style={{ display: 'flex', flexDirection: 'column', flex: 1, overflow: 'hidden' }}>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '12px 13px 14px' }}>
        ...
        {SUBS.map(s => (
          <div key={s.name} style={{ background: 'white', borderRadius: RC, border: '2px solid #F0EDE8', padding: '12px 14px', marginBottom: 8, display: 'flex', alignItems: 'center', gap: 10 }}>
            ...
            <div style={{ fontSize: 12, fontWeight: 900, color: s.countdownColor }}>{s.countdown}</div>
          </div>
        ))}
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginBottom: 12 }}>
          {([{icon:'\u{1F3AC}',name:'Netflix',price:'13,99\u20AC/mois',status:'Actif',color:'#10B981'},{icon:'\u{1F3B5}',name:'Spotify',price:'9,99\u20AC/mois',status:'Peu utilis\u00E9',color:'#E8A820'},{icon:'\u{1F4F1}',name:'Forfait SFR',price:'19,99\u20AC/mois',status:'Actif',color:'#10B981'},{icon:'\u{1F3CB}\uFE0F',name:'Basic Fit',price:'19,99\u20AC/mois',status:'Pas utilis\u00E9',color:'#DC2626'}] as {icon:string;name:string;price:string;status:string;color:string}[]).map(sub => (
            <div key={sub.name} style={{ background: 'white', borderRadius: RC, border: '2px solid #F0EDE8', padding: '10px 12px' }}>
              <div style={{ fontSize: 18, marginBottom: 4 }}>{sub.icon}</div>
              <div style={{ fontSize: 12, fontWeight: 900, color: '#1A1033' }}>{sub.name}</div>
              <div style={{ fontSize: 11, color: '#B0ADA8' }}>{sub.price}</div>
              <div style={{ fontSize: 10, fontWeight: 800, color: sub.color, marginTop: 4 }}>\u25CF {sub.status}</div>
            </div>
          ))}
        </div>
  ...
  const BottomNav = () => {
    const tabs = [
      {id:'home' as NavTab,label:'Accueil',icon:'\u{1F3E0}',screen:'home' as Screen},
      {id:'carnet' as NavTab,label:'Mes lieux',icon:'\u{1F4CD}',screen:'carnet' as Screen},
      {id:'depenses' as NavTab,label:'Compta',icon:'\u{1F9FE}',screen:'depenses' as Screen,color:'#10B981'},
      {id:'abonnements' as NavTab,label:'Contrats',icon:'\u{1F514}',screen:'abonnements' as Screen},
      {id:'profile' as NavTab,label:'Moi',icon:'\u{1F392}',screen:'profile' as Screen},
    ]
    return (
      <div style={{ height: 62, background: 'white', borderTop: '2px solid #F0EDE8', display: 'flex', flexShrink: 0 }}>
        {tabs.map(tab => {
          const isActive = activeNav === tab.id
          const col = tab.color || A
          return (
            <button key={tab.id} onClick={() => navTo(tab.id, tab.screen)} style={{ flex: 1, background: 'none', border: 'none', cursor: 'pointer', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 2, fontFamily: 'inherit', padding: 0 }}>
              <div style={{ fontSize: 20 }}>{tab.icon}</div>
              <div style={{ fontSize: 9, fontWeight: 800, color: isActive ? col : '#B0ADA8' }}>{tab.label}</div>
              {isActive && <div style={{ width: 4, height: 4, borderRadius: '50%', background: col }} />}
            </button>
          )
        })}
      </div>
      ...
      <div style={{ minHeight: '100vh', display: 'flex', justifyContent: 'center', background: '#1A1033', fontFamily: "'Nunito', sans-serif" }}>
        <div style={{ width: '100%', maxWidth: 430, minHeight: '100vh', background: BG, display: 'flex', flexDirection: 'column', position: 'relative', overflow: 'hidden' }}>
          <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
            ...
            {screen === 'depenses' && <DepensesScreen />}
            {screen === 'carnet' && <CarnetScreen />}
            {screen === 'profile' && <ProfileScreen />}
            {screen === 'notifs' && <NotifsScreen />}
            {screen === 'abonnements' && <AbonnementsScreen />}
          </div>
          <BottomNav />
          {showCel && (
            <div style={{ position: 'absolute', inset: 0, background: `linear-gradient(135deg, ${A}, ${AD})`, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', zIndex: 100, textAlign: 'center', padding: 24 }}>
              ...
              <button onClick={() => { setShowCel(false); go('home') }} style={{ background: 'white', color: A, fontSize: 16, fontWeight: 900, padding: '14px 32px', borderRadius: 100, border: 'none', cursor: 'pointer', fontFamily: 'inherit' }}>Super !</button>
            </div>
          )}
          {showDepCel && (
            <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(135deg, #10B981, #059669)', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', zIndex: 100, textAlign: 'center', padding: 24 }}>
              <div style={{ fontSize: 72, marginBottom: 16 }}>{'\u2705'}</div>
              <div style={{ fontSize: 28, fontWeight: 900, color: 'white', marginBottom: 8 }}>D\u00E9pense enregistr\u00E9e !</div>
              <div style={{ fontSize: 16, color: 'rgba(255,255,255,.75)', fontWeight: 700, marginBottom: 32 }}>Ticket sauvegard\u00E9 \u00B7 TVA calcul\u00E9e automatiquement {'\u{1F9EE}'}</div>
              <button onClick={() => setShowDepCel(false)} style={{ background: 'white', color: '#10B981', fontSize: 16, fontWeight: 900, padding: '14px 32px', borderRadius: 100, border: 'none', cursor: 'pointer', fontFamily: 'inherit' }}>Voir l&apos;historique</button>
            </div>
          )}
        </div>
      </div>
